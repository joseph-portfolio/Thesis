...
const char* pi_ip = "raspberrypi.local";
...
void setup() {
  ...
  server.on("/vlv_on",   []() { pnv(3); server.send(200, "text/plain", "Valve Activate"); });
  server.on("/vlv_off",  []() { pnv(4); server.send(200, "text/plain", "Valve Deactivate"); });
...
void cnc() {
  ...
  // Capture Image with UV led
  digitalWrite(uv, HIGH);
  triggerPi();
  delay(15000);
  digitalWrite(uv, LOW);
  
  // Capture Image with white led
...
void pnv(int p) {
  switch (p) {
    case 0:
    digitalWrite(pm1, HIGH); delay(1000); digitalWrite(pm1, LOW);
    break;
    case 1:
    digitalWrite(pm2, HIGH); delay(1000); digitalWrite(pm2, LOW);
    break;
    case 2:
    digitalWrite(pm3, HIGH); delay(1000); digitalWrite(pm3, LOW);
    break;
    case 3:
    digitalWrite(vlv, HIGH);
    break;
...
void handleRoot() {
  ...
  <style>
    ...
    <button onclick="sendCmd('vlv_on')">Valve On</button><br>
    <button onclick="sendCmd('vlv_off')">Valve Off</button><br>
    ...
    <h3>Log Output:</h3>
    <div id="logs" style="font-family: monospace; white-space: 
